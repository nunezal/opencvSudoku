...
def myarray2str(array):
  a_string = ''
  for integers in array:
    strings = [str(integer) for integer in integers]
    a_string = a_string + "".join(strings)
  return a_string
...
def myGrid(values):
    ...
    for v in values.values():
        mygrid.append(int(v))
    new_list = [mygrid[i:i + 9] for i in range(0, len(mygrid), 9)]
    return new_list
...
def solved(values):
    ...
    return values is not {} and all(unitsolved(unit) for unit in unitlist)
